...
export default function MotivationalBoard() {
  ...
  return (
    <div className="w-56 md:w-72 bg-[#0a0a0a] border-[6px] border-[#2c1d1a] rounded-lg p-3 shadow-[inset_0_0_15px_rgba(0,0,0,1),0_20px_25px_rgba(0,0,0,0.8)] relative overflow-hidden">
      ...
      {/* Screen grows with content (min-height, no clipping) so the full quote always shows. */}
      <div className="min-h-[60px] flex items-center justify-center px-2 py-1">
        <p
          className="font-mono text-[#4ade80] font-bold text-center text-[13px] md:text-sm leading-snug drop-shadow-[0_0_8px_rgba(74,222,128,0.6)]"
          // Inline styles beat the unlayered global `p { white-space: nowrap }`
          // (in Tailwind v4 unlayered rules win over utility classes), so the quote always wraps.
